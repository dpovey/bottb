...
if (!id) {
  console.error('usage: ig-publish.mjs <container_id>')
  process.exit(1)
}
...
if (st.status_code !== 'FINISHED')
  throw new Error('container not ready: ' + st.status_code)
const pub = await j(`${G}/${IG}/media_publish`, {
  method: 'POST',
  body: q({ creation_id: id, access_token: TOK }),
})
const info = await j(
  `${G}/${pub.id}?fields=permalink,timestamp,media_type&access_token=${TOK}`
)
