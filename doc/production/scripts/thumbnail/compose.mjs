...
GlobalFonts.registerFromPath(path.join(REPO, 'public/fonts/jost-latin.woff2'), 'Jost')
export { REPO }

export const YT_W = 1920, YT_H = 1080
const FONT_FAMILY = "Jost"
const SAFE_X = 0.06, SAFE_TOP = 0.07, SAFE_BOTTOM = 0.115
const MIN_TYPE = { hero: 0.06, primary: 0.038, secondary: 0.032, label: 0.024 }
const safeInsets = (w, h) => ({ x: Math.round(w*SAFE_X), top: Math.round(h*SAFE_TOP), bottom: Math.round(h*SAFE_BOTTOM) })

function drawCover(ctx, src, sw, sh, dx, dy, dw, dh, fx=0.5, fy=0.5){
  if(!sw||!sh) return
  const scale = Math.max(dw/sw, dh/sh), w = sw*scale, h = sh*scale
  ctx.drawImage(src, dx+(dw-w)*fx, dy+(dh-h)*fy, w, h)
}
const natural = (img) => ({ w: img.naturalWidth || img.width || 1, h: img.naturalHeight || img.height || 1 })
function fitContain(nw, nh, mw, mh){ if(!nw||!nh) return {w:mw,h:mh}; const s=Math.min(mw/nw, mh/nh); return {w:nw*s, h:nh*s} }
...
export function trimTransparent(img){
  ...
  for(let y=0;y<h;y++) for(let x=0;x<w;x++){
    if(data[(y*w+x)*4+3] > 8){ if(y<top)top=y; if(y>bottom)bottom=y; if(x<left)left=x; if(x>right)right=x }
  }
...
function drawLogoRow(ctx, logos, { x, centerY, align, maxW, maxH, gap }){
  ...
  const aspects = logos.map(i => { const {w,h}=natural(i); return w/h })
  const squarest = Math.min(...aspects)
  let sizes = aspects.map(a => fitContain(a, 1, maxW, maxH*Math.pow(squarest/a, WIDTH_PENALTY)))
  const totalGap = gap*(sizes.length-1)
  const rowW = sizes.reduce((s,v)=>s+v.w,0)+totalGap
  if(rowW>maxW){ const s=(maxW-totalGap)/(rowW-totalGap); sizes = sizes.map(v=>({w:v.w*s,h:v.h*s})) }
  ...
  logos.forEach((img,i)=>{ const {w,h}=sizes[i]; ctx.drawImage(img, cursor, bottomY-h, w, h); cursor += w+gap })
  return finalW
}
...
function wrapLines(ctx, text, maxWidth, maxLines){
  ...
  for(let i=1;i<words.length;i++){
    ...
    if(ctx.measureText(cand).width<=maxWidth || lines.length===maxLines-1) cur=cand
    else { lines.push(cur); cur=words[i] }
  }
  lines.push(cur); return lines.slice(0,maxLines)
}
function fitFont(ctx, text, weight, startSize, maxWidth, minSize=18){
  let size=startSize; ctx.font=`${weight} ${size}px ${FONT_FAMILY}`
  while(size>minSize && ctx.measureText(text).width>maxWidth){ size-=2; ctx.font=`${weight} ${size}px ${FONT_FAMILY}` }
  return size
}
function fitArtist(ctx, artist, h, maxWidth){
  const start=Math.round(h*0.125), floor=Math.round(h*MIN_TYPE.hero)
  const oneLine=fitFont(ctx,artist,700,start,maxWidth,Math.max(floor,Math.round(start*0.8)))
  ctx.font=`700 ${oneLine}px ${FONT_FAMILY}`
  if(ctx.measureText(artist).width<=maxWidth) return {size:oneLine, lines:[artist]}
...
function drawAdornments(ctx, w, h, content){
  ...
  const artist=(content.artist||'').trim(), song=(content.song||'').trim()
  const maxTextW = w - safe.x*2
  ctx.textAlign='left'; ctx.textBaseline='alphabetic'
  ctx.shadowColor='rgba(0,0,0,0.85)'; ctx.shadowBlur=Math.round(h*0.014)
  ctx.shadowOffsetX=0; ctx.shadowOffsetY=Math.round(h*0.006); ctx.fillStyle='#ffffff'
  let baseline = h - safe.bottom
  if(song){
    const size=fitFont(ctx,song,500,Math.round(h*0.078),maxTextW,Math.round(h*MIN_TYPE.primary))
    ctx.font=`500 ${size}px ${FONT_FAMILY}`; ctx.fillText(song, safe.x, baseline); baseline -= size*1.25
  }
  const version = content.version && content.version.trim()
  if(version){
    const size=fitFont(ctx,version,500,Math.round(h*0.048),maxTextW,Math.round(h*MIN_TYPE.label))
    ctx.font=`500 ${size}px ${FONT_FAMILY}`
    ctx.fillStyle='rgba(255,255,255,0.78)'; ctx.fillText(version, safe.x, baseline); ctx.fillStyle='#ffffff'
    baseline -= size*1.5
  }
  if(artist){
    const {size, lines}=fitArtist(ctx, artist, h, maxTextW)
    ctx.font=`700 ${size}px ${FONT_FAMILY}`
    for(let i=lines.length-1;i>=0;i--){ ctx.fillText(lines[i], safe.x, baseline); baseline -= size*1.12 }
  }
  ctx.shadowColor='transparent'; ctx.shadowBlur=0; ctx.shadowOffsetY=0
}

export function composeYouTube(ctx, source, sw, sh, content, focusX=0.5, focusY=0.5){
  ctx.clearRect(0,0,YT_W,YT_H)
  if(source) drawCover(ctx, source, sw, sh, 0,0, YT_W, YT_H, focusX, focusY)
  else { ctx.fillStyle='#0a0a0a'; ctx.fillRect(0,0,YT_W,YT_H) }
  const top = ctx.createLinearGradient(0,0,0,YT_H*0.3)
  top.addColorStop(0,'rgba(0,0,0,0.45)'); top.addColorStop(1,'rgba(0,0,0,0)')
  ctx.fillStyle=top; ctx.fillRect(0,0,YT_W,YT_H*0.3)
  const bottom = ctx.createLinearGradient(0,YT_H*0.45,0,YT_H)
  bottom.addColorStop(0,'rgba(0,0,0,0)'); bottom.addColorStop(1,'rgba(0,0,0,0.8)')
  ctx.fillStyle=bottom; ctx.fillRect(0,YT_H*0.45,YT_W,YT_H*0.55)
