...
if (!folder) { console.error('usage: social-photos.mjs <folder> [--when <iso>] [--dry]'); process.exit(1) }

const post = JSON.parse(fs.readFileSync(path.join(folder, 'post.json'), 'utf8'))
const caption = fs.readFileSync(path.join(folder, 'caption.fb.txt'), 'utf8').trim()
...
console.log(`post: ${post.post}  slides: ${post.images.length}  scheduled: ${when || 'now'}`)
console.log('--- caption ---'); console.log(caption); console.log('---------------')
...
for (const im of post.images) {
  ...
  const up = await put(`social/brisbane-2026/photos/${post.post}/${im.file}`, fs.readFileSync(local), {
    access: 'public', token: env.BLOB_READ_WRITE_TOKEN, contentType: 'image/jpeg',
...
for (const u of urls) {
  const r = await j(`${G}/${PAGE}/photos`, {
    method: 'POST',
    body: q({ url: u.url, published: 'false', alt_text_custom: u.alt, access_token: TOK }),
  })
  fbids.push(r.id)
...
if (when) {
  ...
  body.scheduled_publish_time = String(Math.floor(new Date(when).getTime() / 1000))
}
...
const check = await j(`${G}/${res.id}?fields=id,created_time,is_published,permalink_url&access_token=${TOK}`)
...
if (process.argv.includes('--ig')) {
  ...
  const igCaption = fs.readFileSync(path.join(folder, 'caption.ig.txt'), 'utf8').trim()
  const children = []
  for (let i = 0; i < urls.length; i++) {
    const c = await j(`${G}/${IG}/media`, {
      method: 'POST',
      body: q({ image_url: urls[i].url, is_carousel_item: 'true', alt_text: urls[i].alt, access_token: TOK }),
    })
    children.push(c.id)
  ...
  const parent = await j(`${G}/${IG}/media`, { method: 'POST', body: q(parentBody) })
  ...
  for (let i = 0; i < 30; i++) {
    const st = await j(`${G}/${parent.id}?fields=status_code&access_token=${TOK}`)
    if (st.status_code === 'FINISHED') break
    if (st.status_code === 'ERROR') throw new Error(`container ${parent.id} errored`)
  ...
  const info = await j(`${G}/${pub.id}?fields=permalink,timestamp&access_token=${TOK}`)
