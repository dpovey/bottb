...
| 2.3 | Sweep ad-hoc inputs onto the shared `<FormField>`/`<Input>` in `src/components/ui/form.tsx` (**done** — the `Admin*` family was removed, not extended) | ~30 instances of `w-full px-4 py-2 rounded-lg bg-white/5 border border-white/20 …` in admin clients                                                                                                                                          |
...
**Exit:** four admin clients each shrink by ≥ 30%. Zero raw `<input className="w-full px-4 py-2 …">` in admin code (the input half is done; the `useAdminCRUD` shrink is not).
