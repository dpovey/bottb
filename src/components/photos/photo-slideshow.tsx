...
export function PhotoSlideshow({
  ...
  // Get shareable URL for current photo (uses dedicated page with OG meta tags)
  const getShareUrl = () => {
    const photo = allPhotos[currentIndex];
    if (!photo) return "";
  ...
  const handleShareTwitter = () => {
    const url = encodeURIComponent(getShareUrl());
    const text = encodeURIComponent("Check out this photo from Battle of the Tech Bands! 🎸");
    window.open(
              ...
              <div className="relative">
                ...
                {showShareMenu && (
                  <>
                    ...
                    />
                    <div className="absolute right-0 top-full mt-2 w-48 bg-bg-elevated border border-white/10 rounded-lg shadow-xl z-50 py-1 overflow-hidden">
                      <button
                        onClick={handleShareLinkedIn}
                        className="w-full px-4 py-2.5 text-left text-sm text-text-muted hover:text-white hover:bg-white/5 transition-colors flex items-center gap-3"
                      >
