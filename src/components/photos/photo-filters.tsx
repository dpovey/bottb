...
export function PhotoFilters({
  ...
  if (selectedCompanySlug && !selectedEventId) {
    ...
    displayBands = filteredBands.map((b) => ({
      id: b.id,
      name: b.name,
      ids: [b.id],
    }))
  ...
  const selectedBandName =
    selectedBandId === 'none'
      ...
      : selectedBandId && selectedBandId.startsWith('bandIds:')
        ? displayBands.find(
            (db) => db.id === selectedBandId.split(':')[1]?.split(',')[0]
          )?.name || 'Band'
  ...
  return (
    <FilterBar>
      ...
      >
        <option value="" key="all-photographers-default">
          All Photographers
        </option>
        {photographers.map((photographer) => {
          const isAvailable =
            !availableFilters || availablePhotographerNames.has(photographer)
          return (
            <option
              key={photographer}
              value={photographer}
