...
const EVENT_SCORING_VERSIONS: Record<
  string,
  { scoring_version: string; winner?: string }
> = {
  ...
  try {
    ...
    for (const event of events) {
      ...
      if (!needsVersionUpdate && !needsWinnerUpdate) {
        console.log(
          `  ⏭️  ${event.name} (${event.id}): already has correct config`
        )
        console.log(
          `      Version: ${currentVersion}, Winner: ${currentWinner || 'N/A'}`
        )
        skippedCount++
        continue
      }
