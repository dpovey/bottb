...
const SOURCE_IMAGE = path.join(process.cwd(), "public/images/bottb-lightning.png");
...
async function createCircularFavicon(size: number): Promise<Buffer> {
  ...
  const imgOffsetX = Math.round((workSize - imgWidth) / 2) - Math.round(workSize * 0.016);
...
async function generateFavicons() {
  ...
  console.log("\nNext steps:");
  console.log("1. The metadata in layout.tsx will automatically use src/app/icon.png and src/app/apple-icon.png");
  console.log("2. Update layout.tsx metadata to include manifest and theme color");
}
