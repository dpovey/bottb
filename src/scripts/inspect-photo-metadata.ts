...
async function inspectPhoto(imagePath: string) {
  ...
  if (embeddedMetadata) {
    ...
    console.log(
      `   Show on Social: ${embeddedMetadata.showOnSocial ? '✅ Yes' : '❌ No'}`
    )
    ...
    if (raw.hierarchicalSubject || raw.HierarchicalSubject) {
      ...
      console.log(
        `   hierarchicalSubject: ${JSON.stringify(hs, null, 2).split('\n').join('\n   ')}`
      )
    }
    ...
    if (raw.subject || raw.Subject) {
      ...
      console.log(
        `   dc:subject: ${JSON.stringify(subj, null, 2).split('\n').join('\n   ')}`
      )
    }
  ...
  if (hasSidecar) {
    ...
    if (sidecarMetadata) {
      ...
      console.log(
        `   Show on Social: ${sidecarMetadata.showOnSocial ? '✅ Yes' : '❌ No'}`
      )
  ...
  console.log(
    `   Show on Social: ${combined.showOnSocial ? '✅ Yes' : '❌ No'}`
  )
