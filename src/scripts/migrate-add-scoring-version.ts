  ...
  try {
    ...
    for (const event of events) {
      ...
      if (existingInfo.scoring_version) {
        console.log(
          `⏭️  ${event.name}: Already has scoring_version: ${existingInfo.scoring_version}`
        )
        continue
      }
      ...
      if (event.id === 'sydney-2025') {
        ...
        console.log(
          `🎯 ${event.name}: Setting scoring_version to ${scoringVersion} (Scream-o-meter)`
        )
      } else if (event.id === 'brisbane-2024') {
        scoringVersion = '2022.1'
        winner = 'The Fuggles'
        console.log(
          `🏆 ${event.name}: Setting scoring_version to ${scoringVersion}, winner: ${winner}`
        )
        ...
        console.log(
          `📅 ${event.name}: Setting scoring_version to ${scoringVersion} (Costumes/Backgrounds)`
        )
      }
    ...
    console.log(
      '   - 2022.1: Single winner display (brisbane-2024, brisbane-2025)'
    )
