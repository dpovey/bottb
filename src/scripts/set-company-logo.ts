...
`);
  ...
  try {
    ...
    if (companies.length === 0) {
      ...
      console.log("\nAvailable companies:");
      const { rows: all } = await sql`SELECT slug, name FROM companies ORDER BY name`;
    ...
    const uploadedLogoUrl = await fetchAndUploadLogo(logoUrl, companySlug, "logo");
    
    // Fetch and upload icon
    const uploadedIconUrl = await fetchAndUploadLogo(iconUrl, companySlug, "icon");
