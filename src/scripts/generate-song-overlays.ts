...
const outBase =
  get('out') ?? '/Volumes/Extreme SSD/bottb/events/2026/Brisbane/02_Production'
...
async function main() {
  const { rows: events } =
    await sql`SELECT id, name FROM events WHERE LOWER(name) LIKE ${'%' + eventQ + '%'} ORDER BY date DESC`
  ...
  for (const band of bands) {
    ...
    const { rows: songs } =
      await sql`SELECT s.* FROM setlist_songs s WHERE s.band_id = ${band.id} ORDER BY s.position ASC`
    if (!songs.length) {
      console.log(`-- ${band.name}: no setlist, skipped`)
      continue
    }
    const logoUrls: string[] = (band.companies as any[])
      .map((c) => c.logo_url)
      .filter(Boolean)
    ...
    for (const song of songs) {
      ...
      const file = path.join(
        dir,
        `${String(++n).padStart(2, '0')}-${slug(credit.title)}.png`
      )
