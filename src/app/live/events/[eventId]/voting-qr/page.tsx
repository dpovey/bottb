...
export default function VotingQRPage() {
  const params = useParams();
  const eventId = params.eventId as string;
  const [event, setEvent] = useState<Event | null>(null);
  const [crowdQRCode, setCrowdQRCode] = useState("");
  ...
  useEffect(() => {
    if (event) {
      const generateQRCode = async () => {
        ...
        const crowdUrl = `${baseUrl}/vote/crowd/${eventId}`;

        try {
          const crowdQR = await QRCode.toDataURL(crowdUrl, {
            width: 400,
            margin: 2,
            color: {
              dark: "#000000",
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 text-center">
            <div className="mb-6">
              ...
              <p className="text-gray-300 text-lg">
                Scan to vote for your favorite band
              </p>
            </div>
            ...
            <div className="mt-4">
              <p className="text-gray-400 text-sm">
                One vote per person • Simple band selection
              </p>
            </div>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start">
                ...
                <span className="text-gray-300 text-lg">
                  Select your favorite band from the list
                </span>
              </div>
              <div className="flex items-start">
                <span className="bg-slate-600 text-white rounded-full w-8 h-8 flex items-center justify-center font-bold mr-4 shrink-0 text-sm">
