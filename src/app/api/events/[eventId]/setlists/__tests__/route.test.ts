...
const mockUpdateConflictStatus = updateConflictStatus as ReturnType<typeof vi.fn>;
...
describe("/api/events/[eventId]/setlists", () => {
  ...
  describe("GET", () => {
    ...
    it("returns all setlists grouped by band with conflicts", async () => {
      ...
      const request = new NextRequest(`http://localhost/api/events/${eventId}/setlists`);
  ...
  describe("POST (Admin)", () => {
    const createAdminRequest = (eventId: string, body: Record<string, unknown>) => {
    ...
    it("returns 401 when not authenticated as admin", async () => {
      ...
      const request = createAdminRequest("event-1", { action: "lock", bandId: "band-1" });
    ...
    it("refreshes conflict status", async () => {
      ...
      const request = createAdminRequest(eventId, { action: "refresh_conflicts" });
      const response = await POST(request, {
        params: Promise.resolve({ eventId }),
      });
    ...
    it("validates action parameter", async () => {
      ...
      const request = createAdminRequest("event-1", { action: "invalid_action" });
      const response = await POST(request, {
        params: Promise.resolve({ eventId: "event-1" }),
      });
