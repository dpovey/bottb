...
const mockGetFinalizedResults = getFinalizedResults as unknown as ReturnType<
  typeof vi.fn
>
const mockGetPhotosByLabel = getPhotosByLabel as unknown as ReturnType<
  typeof vi.fn
>
...
describe('ResultsPage', () => {
  ...
  it('displays complete results table for 2025.1 scoring', async () => {
    ...
    expect(
      screen.getByRole('columnheader', { name: 'Song' })
    ).toBeInTheDocument()
    expect(
      screen.getByRole('columnheader', { name: 'Perf' })
    ).toBeInTheDocument()
    expect(
      screen.getByRole('columnheader', { name: 'Vibe' })
    ).toBeInTheDocument()
    expect(
      screen.getByRole('columnheader', { name: 'Vote' })
    ).toBeInTheDocument()
    expect(
      screen.getByRole('columnheader', { name: 'Noise' })
    ).toBeInTheDocument() // 2025.1 has Noise
    expect(
