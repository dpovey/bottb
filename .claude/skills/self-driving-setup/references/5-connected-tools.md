...
1. Ask **once**, multi-select. **"None of these" is the first option** (the safe default — an accidental `enter` declines); order the _tools_ after it, seeding with any step-2 hints so a tool you saw evidence of comes first among them:
